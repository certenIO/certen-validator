...
func (r *ProofArtifactRepository) CreateProofArtifact(ctx context.Context, input *NewProofArtifact) (*ProofArtifact, error) {
	...
	query := `
		INSERT INTO proof_artifacts (
			...
			batch_id, anchor_id, anchor_tx_hash, anchor_block_number, anchor_chain,
			merkle_root, leaf_hash, leaf_index, merkle_path,
	...
	proof.AnchorID = input.AnchorID
	proof.AnchorTxHash = input.AnchorTxHash
	proof.AnchorBlockNumber = input.AnchorBlockNumber
	proof.AnchorChain = input.AnchorChain
	...
	err := r.db.QueryRowContext(ctx, query,
		...
		input.BatchID, input.AnchorID, input.AnchorTxHash, input.AnchorBlockNumber, input.AnchorChain,
		input.MerkleRoot, input.LeafHash, input.LeafIndex, merklePathJSON,
...
func (r *ProofArtifactRepository) UpdateProofVerified(ctx context.Context, proofID uuid.UUID, verified bool) error {
	...
	result, err := r.db.ExecContext(ctx, query, status, string(status), proofID)
	if err != nil {
