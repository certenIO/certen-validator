/**
 * Fix CertenAnchorV4 BLS Keys
 *
 * The V4 anchor contract was deployed with empty BLS public keys for all 7 validators.
 * This script removes each validator and re-registers them with the correct
 * deterministic BLS keys from the MASTER backup.
 *
 * Usage:
 *   OWNER_PRIVATE_KEY=0x... node fix_v4_bls_keys.js
 *
 * Owner: 0x8B18BE5EE7B4e1f33BAd6f5f0f31588F64F63A4e
 */
...
// CertenAnchorV4 on Sepolia
const CONTRACT_ADDRESS = '0x813e7F7EF2FfC14Aa9E86e2ababC49059495eC3C';

// Validators with correct BLS keys (from bls_keys_backup_MASTER.json)
const VALIDATORS = [
  {
    name: 'validator-1',
...
async function main() {
  ...
  console.log('Fix CertenAnchorV4 BLS Keys');
  console.log('='.repeat(70));
  console.log('');

  if (!OWNER_PRIVATE_KEY) {
    console.error('ERROR: OWNER_PRIVATE_KEY not set!');
    console.error('Usage: OWNER_PRIVATE_KEY=0x... node fix_v4_bls_keys.js');
    process.exit(1);
  }
  ...
  const countBefore = await contract.getValidatorCount();
  const tvpBefore = await contract.totalVotingPower();
  console.log('Validator Count:', countBefore.toString());
  console.log('Total Voting Power:', tvpBefore.toString());
  console.log('');

  // Process each validator: remove then re-register with BLS key
  let success = 0;
  let failed = 0;
  ...
  for (const v of VALIDATORS) {
    ...
    const currentKeyLen = info.blsPublicKey ? (info.blsPublicKey.length - 2) / 2 : 0;
    console.log(`  Current: registered=${info.registered}, blsKey=${currentKeyLen} bytes`);

    if (!info.registered) {
      console.log('  Not registered, registering fresh...');
    } else {
      // Remove existing registration (with empty BLS key)
      console.log('  Removing (has empty BLS key)...');
      try {
    ...
    // Register with correct BLS key
    console.log('  Registering with BLS key...');
    console.log('  Key: ' + v.blsKey.slice(0, 20) + '...' + v.blsKey.slice(-16));
    ...
    // Re-add as operator if needed
    try {
      const isOp = await contract.isOperator(v.address);
      if (!isOp) {
        console.log('  Re-adding as operator...');
        const opTx = await contract.addOperator(v.address, { gasLimit: 100000n });
        await opTx.wait();
        console.log('  Operator re-added');
      } else {
        console.log('  Already an operator');
      }
    } catch (e) {
      console.log('  Operator check/add: ' + e.message.slice(0, 80));
    }

    // Verify
    const newInfo = await contract.validators(v.address);
    const keyMatch = newInfo.blsPublicKey.toLowerCase() === v.blsKey.toLowerCase();
    console.log('  Verified: key ' + (keyMatch ? 'MATCHES' : 'MISMATCH!'));
  ...
  const countAfter = await contract.getValidatorCount();
  const tvpAfter = await contract.totalVotingPower();
  console.log('Validator Count:', countAfter.toString());
  console.log('Total Voting Power:', tvpAfter.toString());
  console.log('');

  for (const v of VALIDATORS) {
    const info = await contract.validators(v.address);
    const keyLen = info.blsPublicKey ? (info.blsPublicKey.length - 2) / 2 : 0;
    const keyMatch = info.blsPublicKey.toLowerCase() === v.blsKey.toLowerCase();
    const status = keyMatch ? 'OK' : 'FAIL';
    console.log(`  ${status} ${v.name}: ${keyLen} bytes BLS key`);
  }

  console.log('');
  console.log(`Success: ${success}/${VALIDATORS.length}, Failed: ${failed}/${VALIDATORS.length}`);
}
